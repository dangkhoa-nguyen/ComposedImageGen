...
parser = argparse.ArgumentParser(description="Generate images with SDXL using embeddings.")
...
parser.add_argument("--batch_size", type=int, default=1, help="Batch size for DataLoader and batched generation")
...
if not os.path.isdir(args.text_embeddings_dir):
    raise FileNotFoundError(args.text_embeddings_dir)

# Data
dataset = ComposedEmbedsDataset(text_embeddings_dir=args.text_embeddings_dir)
if len(dataset) == 0:
    raise RuntimeError("No embedding (.pt) files found.")
