...
def circo_generate_test_dict(relative_test_dataset: CIRCODataset, clip_model: CLIP, index_image_features: torch.Tensor, index_text_features: Optional[torch.Tensor],
    ...
    if alpha < 1.0:
        ...
        # Hybrid gallery feature 
        index_composed_features = F.normalize((alpha * index_image_features + (1.0 - alpha) * index_text_features), dim=-1)
        
    else:   # alpha = 1.0 -> original image-only baseline
        index_composed_features = index_image_features
    
    # Compute similarity with the hybrid gallery feature 
    similarity = predicted_features @ index_composed_features.T
...
def main():
    ...
    parser.add_argument("--alpha", type=float, default=1.0,
                        help="Weight for the original gallery image feature in hybrid retrieval. "
                             "alpha=1.0 uses only gallery image features (baseline); "
                             "alpha=0.0 uses only gallery textualized features; "
                             "0<alpha<1 combines both as image_feature + (1-alpha)*textualized_feature.")
